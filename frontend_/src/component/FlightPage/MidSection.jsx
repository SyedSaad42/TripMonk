...
const MidSection = ({ moveToNextTab, updateTripType}) => {
  const navigate = useNavigate();

  const { departure, arrival, departure_date, return_date, tripType , setTripType} = useContext(SearchContext);
  const { flights, setFlights, selectedFlight, setSelectedFlights } = useContext(FlightContext);
 ...
 // Progress animation logic
  ...
  const handleSubmit = (flight) => {
   ...
   addToTripCart("departure", flight);

    if(updateTripType){
      updateTripType(tripType);
      console.log("Updated trip type: " , tripType);
    }
    
  
    // addToTripCart({ type: "Departure Flight", data: flight }); // 👈 Add to cart
    if (moveToNextTab) {
      moveToNextTab();
    } else {
      navigate("/tripcart");
